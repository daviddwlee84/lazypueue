...
Source and standalone archive updates require a formal stable GitHub release. Standalone release archives download the exact platform archive and verify checksums before candidate inspection and replacement. Source installations build its exact tag with the installed Go command. Until a release exists, those checks report `source-unavailable`; they do not substitute the main branch or change the checkout. Version output uses an injected build version, then Go's embedded module version, then `dev`.

The source package is `github.com/daviddwlee84/lazypueue` at the repository root. Release archives cover macOS/Linux/Windows amd64/arm64; standalone atomic replacement remains Unix-only. Windows Scoop handoff and status polling are documented in the [Windows guide](../README.md#windows-installation-and-upgrades). Archive updates require no Go installation; an absent archive, failed download, or checksum mismatch never falls back to source. Go keeps the user's `GOTOOLCHAIN` policy, which may download a compatible toolchain. A missing Go command is reported rather than installed automatically.

Inspection separates build provenance from package ownership. A Homebrew installation can delegate to the owning `brew upgrade <exact-formula>` after the usual terminal confirmation or `--yes`. The keg receipt supplies the installed formula and tap; read-only queries must prove that the selected `brew` owns the same Cellar and stable `opt` prefix. Missing or conflicting evidence produces manual guidance. This never runs upgrade-all, sudo, or direct replacement of a managed executable. Nix, mise, and unknown installations continue to receive guidance. Recognized unmanaged local/development builds need `--force`; that flag never bypasses ownership, executable identity, or candidate verification.

For Homebrew, `--check` and `--dry-run` show the exact command without upgrading or consulting GitHub latest. Homebrew selects its available formula version when apply runs. After success, lazypueue inspects the owning formula's stable `opt` target, so a removed old keg or another copy on PATH cannot redirect verification. JSON retains the original `installation` and reports `manager_command`, `installed_version`, `installed_path`, `installed_resolved_path`, and either `managed-updated` or `managed-unchanged`; `latest_version` remains absent and `update_available_known` is false. A no-op reports the actual installed version and notes that the tap may lag GitHub releases. Cancellation, failure, or failed post-upgrade verification returns an error; Homebrew may have completed some work, and lazypueue does not claim rollback or switch to archive/source replacement. CLI progress uses stderr (discarded for JSON); the TUI shows the final verified result without child output corrupting its display.

Source/archive apply builds in private staging next to the resolved current executable, validates package/module/platform/version, rechecks the original file and launch symlinks, and publishes with an atomic rename. A different copy earlier on PATH or a changed GOBIN cannot redirect the update. Failed builds, cancellation, concurrent updates, and changed destinations retain the original. Start a new invocation to use the updated binary.
